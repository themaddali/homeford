	...
	Croppic.prototype = {
			...
			that.form.find('input[type="file"]').change(function() {
				...
				$.ajax({
					...
					if (response.status == 'success') {
						...
						that.obj.append('<img src="' + '/homeford/api/profileupload/picture/' + data.files[0].id + '">');
						that.initCropper();
		...
		initDrag : function() {
			...
			that.img.on("mousedown", function(e) {
				...
				var z_idx = that.img.css('z-index'), drg_h = that.img.outerHeight(), drg_w = that.img.outerWidth(), pos_y = that.img.offset().top + drg_h - e.pageY, pos_x = that.img.offset().left + drg_w - e.pageX;
